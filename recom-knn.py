...
SIMILARITIES = ["cosine", "pearson","cosine_jaccard","cosine_salton"]
...
def pairwise_salton_ochiai_sparse(csr):
    #print(csr)
    csr = csr_matrix(csr).astype(bool).astype(int)
    #print(type(csr))
    #print(csr)
    ...
    cardinality_multiplication = csr_matrix.copy(intrsct)
    #print(type(cardinality_multiplication))
    #print(np.shape(cardinality_multiplication))
    #print(cardinality_multiplication)

    rows,cols = cardinality_multiplication.nonzero()      
    for row,col in zip(rows,cols):
        cardinality_multiplication[row,col] = cardinality_multiplication[row,col] / math.sqrt((intrsct[col,col] * intrsct[row,row]))
    ...
    #out = csr_matrix((data, indices, indptr), intrsct.shape)
    return cardinality_multiplication
...
class ItemKNN(Recommender):
    ...
    * Aggarwal, C. C. (2016). Recommender systems (Vol. 1). Cham: Springer International Publishing.
    ...
    def fit(self, train_set, val_set=None):
        ...
        if self.similarity in ("cosine_jaccard","cosine_salton"):
            similarity_temp = self.similarity
            self.similarity = "cosine" 
            self.sim_mat_rating_based = compute_similarity(
            weight_mat, k=self.k, num_threads=self.num_threads, verbose=self.verbose
            )
            self.similarity = similarity_temp
            if self.similarity == "cosine_jaccard": 
                self.sim_mat_struct_based = pairwise_jaccard_sparse(weight_mat)
            elif self.similarity == "cosine_salton":
                self.sim_mat_struct_based = pairwise_salton_ochiai_sparse(weight_mat)

            sim_mat_struct_based_powered = csr_matrix.power(self.sim_mat_struct_based , 2.6 ) 
            self.sim_mat = csr_matrix.multiply(sim_mat_struct_based_powered,self.sim_mat_rating_based)
        ...
        #print(type(self.sim_mat_rating_based))
        #print(self.sim_mat_rating_based)
        #print(np.shape(self.sim_mat_rating_based))
        #print("aaaaaaaaaaaa")

        #weight_mat_not_centered = self.train_set.matrix.copy()
        #weight_mat_not_centered = weight_mat_not_centered.T.tocsr()
        #print(type(weight_mat_not_centered))
        #print(weight_mat_not_centered)
        #print(np.shape(weight_mat_not_centered))
        
        #print(type(self.sim_mat_struct_based))
        #print(self.sim_mat_struct_based)
        #print(np.shape(self.sim_mat_struct_based))
